...
class MainView extends Component {
    ...
    createGame() {
        ...
        const Bet = getBetInstance(true)
        const number = this.state.number % 256

        return Bet.methods.saltedHash(number, this.state.salt).call()
            .then(hash => {
                ...
                return Bet.methods.createGame(hash, this.state.nick).send({ value, from: this.props.accounts[0] })
            }).then(tx => {
    ...
    acceptGame() {
        ...
        this.acceptForm.validateFields((err, values) => {
            ...
            // TRANSACTION
            return Bet.methods.acceptGame(game.id, values.number, values.nick)
    ...
    renderGameList() {
        return <div className="card">
            <h1 className="light">Bet</h1>
            <p className="light">Bet is an Ethereum distributed app. Select a game to join or create a new one.</p>
